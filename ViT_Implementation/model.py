...
class ViT(nn.Module):
    def __init__(self, chw = (3, 224, 224), n_patches = 8, n_blocks=4, hidden_dim = 16, n_heads=4, n_classes=1000):
...
class MSA(nn.Module):
    ...
    def forward(self, sequences):
        ...
        Q = Q.view(N, seq_len, self.n_heads, self.d_head).transpose(1, 2)  # (N, n_heads, seq_len, d_head)
        K = K.view(N, seq_len, self.n_heads, self.d_head).transpose(1, 2)  # (N, n_heads, seq_len, d_head)
        V = V.view(N, seq_len, self.n_heads, self.d_head).transpose(1, 2)  # (N, n_heads, seq_len, d_head)
        
        # compute scaled dot-product attention
        attention = self.softmax(Q @ K.transpose(-2, -1) / (self.d_head ** 0.5))  # (N, n_heads, seq_len, seq_len)
